...
| R4 | View + Reference | Reset/zoom grouped; demote the legend to a clean reference key; final toolbar balance pass | **done** |
...
- R4 (final): grouped the View control (#reset) into a subtle pill; demoted the kinds/marks legend to a quiet reference key (hairline rule, quieter text, the gesture-hint italicised); and FIXED the pre-existing overlap — replaced the hardcoded `top:120px` content offset with a flex-column body layout (#content flex:1 wraps #wrap+#chips at inset:0) so the toolbar reserves its height and every control is fully visible + clickable (proven via elementFromPoint: row-2 controls returned the globe before, return themselves after). Caught+fixed a mobile flex nondeterminism (display:contents on ≤760px restores the block layout); smoke 0px 3x. map.js UNTOUCHED across all four R iterations. Baselines re-blessed both platforms.

The globe controls are now three clean clusters (Transport / View+Locate+Scope) over a quiet reference key, with no clipping — reorganized entirely in markup+CSS with zero behaviour change.
