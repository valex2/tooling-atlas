...
| G | 12 | Rootedness in the country layer (Make+Manufacture share as glyph/weight) — risky, careful, last | **done** |
...
- Batch G (item 12): encoded each country's Make+Manufacture share (the 'hardest making') as a subtle hue nudge — the existing count-driven neutral fill blends toward the Make earth-tone by 0.35×share (count still drives alpha, no new SVG layer). All 24 card-countries join to COUNTRIES exactly; high-making (USA 71%, Japan 90%, China 86%) read measurably warmer than low (Netherlands 17%, France 38%). Kept deliberately subtle so it doesn't fight the arcs/dots/labels — a quiet honest signal, not clutter. Baselines re-blessed both platforms.
