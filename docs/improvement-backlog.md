...
| # | Track | Item | Status |
|---|-------|------|--------|
| 1 | a11y | Globe time-lapse (`setInterval` play) ignores `prefers-reduced-motion` — under reduce, snap to full T instead of animating | pending |
| 2 | interaction | Cross-view hash-carry audit: `#card=` / `#hist=` / `#mode=` must behave identically across Home/Globe/Tree/Relay/Browse; fix drift | pending |
