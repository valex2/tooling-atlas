...
| K | polish | Tree lane order by spine (#3) + hover-preview lineage (#4); Browse decade-tail binning (#4) + front-truncation affordance (#6) | **done** |
...
- Batch K (Tree #3,#4 + Browse #4,#6): Tree lanes now group by the four spine histories (Optics→…→Other); hover-preview lifts a node's lineage and quiets the resting web (0.25→0.12); Browse 'By decade' bins the sparse pre-1800 tail (55→26 rows, counts sum to 283); card fronts get a deterministic 'more' expand affordance (front>950 chars → fade+more, ~235/283). NOTE: the front affordance was made DATA-based, not layout-measured — the agent's scrollHeight check was non-deterministic across runs (flaky mobile capture); fixed to key .ov on front length so smoke is stable. Baselines re-blessed both platforms.
