...
**Back.** It made search good, and it made Google. The method was openly [published in 1998](http://infolab.stanford.edu/~backrub/google.html) and the patent belonged to Stanford, so the algorithm itself was never the secret. What Google built that no one could read off the paper was the work of crawling and indexing the entire web, and the machines to do it fast. Stanford, which owned the algorithm outright, sold its share of the patent for [about $336 million](https://en.wikipedia.org/wiki/PageRank); Google, which owned the index, became one of the largest companies on earth.
