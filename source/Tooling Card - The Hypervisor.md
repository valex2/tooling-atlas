...
*Curator's note:* **Placement.** Make cell, IBM Cambridge, 1967, head of the new [[Thread - The Network]]. Goal Both; Mechanism Convergence. Confidence Direct. It is a Make tool (it builds isolated machines), but its payoff is pure Manufacture: the cloud is computing produced and sold at scale on top of this one tool, which is worth saying in the essay. **Analysis.** Pairs against [[Tooling Card - MapReduce]] as the two halves of distributed computing: the hypervisor slices one machine into many, MapReduce fuses many machines into one, and between them the boundary of "a computer" dissolves. The thirty-year gap between invention and payoff is the deck's favorite shape, the delay being the point: a tool can sit finished and idle until the surrounding hardware (cheap x86 servers) makes it matter. It is the technical seed of the compute-concentration story in [[Tooling Leadership Home]]. **Nuance.** The 1967 origin was on an IBM System/360 mainframe, not a microprocessor; the card BuildsOn [[Tooling Card - The Microprocessor]] because the x86 revival is what made it world-shaping, and the mainframe root is noted here. Creasy is the inventor, Rosenblum the reviver, folded into one card as the two carriers of the same tool. CP-40 ran slightly earlier in 1966-67; CP-67 is the widely-used one.
