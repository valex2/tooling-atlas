---
Tags: tooling-card
Name: The Metal-Organic Vapor Reactor
Kind: Manufacture
Tool: grow crystal layers from metal-organic gas
Person: Harold Manasevit
Place: Anaheim, California, USA
City: Anaheim
Lat: 33.84
Lon: -117.91
Era: 1960s
Year: 1968
Goal: Both
Mechanism: Convergence
Confidence: Direct
Unlocked: compound-semiconductor crystal grown to order
Significance: "The metal-organic vapor reactor grows compound crystal atom-layer on atom-layer from gas, and the lead settled not in the published recipe but with the growers who tune the flows and the few firms that build the reactor."
BuildsOn:
  - "[[Tooling Card - The Czochralski Puller]]"
Enables:
  - "[[Tooling Card - The Blue LED]]"
Threads:
  - "[[Thread - Wafer Processing]]"
  - "[[Thread - Critical Materials]]"
  - "[[Thread - Energy]]"
---
*Manufacture · Anaheim, California, USA · 1960s*
**Harold Manasevit** · crystal grown from metal-organic gas

**Front.** In February 1968, a chemist named [Harold Manasevit](https://en.wikipedia.org/wiki/Harold_M._Manasevit) at North American Rockwell's Autonetics division in [Anaheim](https://en.wikipedia.org/wiki/Metalorganic_vapour-phase_epitaxy) wanted to grow a sheet of single-crystal gallium arsenide straight onto a plain insulating slab like sapphire. The usual furnace tricks gave a rough, jumbled film. His fix was a different way to feed the crystal. He held the slab on a [red-hot block of graphite](https://en.wikipedia.org/wiki/Metalorganic_vapour-phase_epitaxy), the susceptor, and ran a carrier gas over it carrying two things: [trimethylgallium](https://en.wikipedia.org/wiki/Metalorganic_vapour-phase_epitaxy), a metal-organic liquid bubbled into vapor, and arsine, a hydride gas. The heat cracks both at the surface, and the loose gallium and arsenic lock into place. The crystal builds up atom-layer on atom-layer, and you set the thickness and the mix by turning gas valves. Manasevit [published the result](https://pubs.aip.org/aip/apl/article-abstract/12/4/156/40950/SINGLE-CRYSTAL-GALLIUM-ARSENIDE-ON-INSULATING) and named the method MOCVD, for the metal-organic gas at its heart.

**Back.** The reactor sat rough for nine years until [Russell Dupuis and Dan Dapkus at Rockwell](https://en.wikipedia.org/wiki/Quantum_well_laser) grew the first device-grade layers with it in 1977 and ran a room-temperature laser that matched crystal grown any other way. After that the reactor grew the crystal under the LED, the laser diode, and the fast radio chip. At a small Japanese company called Nichia, [Shuji Nakamura rebuilt a bought MOCVD machine into a two-flow reactor](https://www.nobelprize.org/uploads/2018/06/nakamura-lecture.pdf), blowing a second gas stream down onto the wafer, and grew the clean gallium nitride that made the blue diode. The reactor-building settled with a few firms: [Aixtron in Germany and Veeco in the United States together take most of a market worth about $1.2 billion](https://www.globalgrowthinsights.com/market-reports/metal-organic-chemical-vapor-deposition-equipment-market-101771), with China now racing to build its own.

