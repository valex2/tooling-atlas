...
let q="",kind=null,thread="",goal="",mech="",sortK="year",sortDir=1;
try{const st=getState();if(st.kind)kind=st.kind;if(st.thread)thread=st.thread;if(st.q)q=st.q;if(st.goal)goal=st.goal;if(st.mech)mech=st.mech;}catch(e){}
...
function render(){
 ...
 try{setState({kind:kind||'',thread:thread||'',q:q||'',goal:goal||'',mech:mech||''});}catch(e){}
...
document.getElementById("reset").onclick=()=>{q="";kind=null;thread="";goal="";mech="";document.getElementById("q").value="";document.getElementById("thread").value="";document.getElementById("goal").value="";document.getElementById("mech").value="";render();};
